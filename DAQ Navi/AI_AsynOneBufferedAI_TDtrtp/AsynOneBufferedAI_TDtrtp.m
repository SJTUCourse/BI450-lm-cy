...
function AsynOneBufferedAI_TDtrtp()
...
triggerSource = Automation.BDaq.SignalDrop.SigExtDigTrigger0;
...
trigger1Action = Automation.BDaq.TriggerAction.DelayToStop;
trigger1Source = Automation.BDaq.SignalDrop.SigExtDigTrigger1;
trigger1Edge = Automation.BDaq.ActiveSignal.RisingEdge;
trigger1DelayCount = int32(10);
trigger1Level = double(5.0);
...
try
    ...
    % source refer to for manual details.
    ...
    %for trigger1
    trigger1 = waveformAiCtrl.Trigger1;
    if ~isempty(trigger1)
...
figure('NumberTitle', 'off', 'Name', 'AsynOneBufferedAI_TDtrtp');
plot(allChanData);
end
